 ...
 *    + �?справлено поведение парсера при установке правила sfgParamsAutoAdd(). Теперь
 ...
 * 1.00
 *  + �?справлен баг с закрывающимися тегами приводящий к созданию непарного тега рушащего вёрстку
 * 1.00 RC2
 ...
 * 0.95
 *  + �?справлено типографирование ?.. и !.. (две точки в конце больше не превращаются в троеточие)
 ...
 * 0.93
 *      + �?справлен баг с удалением пробелов (например в "123 &mdash; 123")
 *  + �?справлена ошибка из-за которой иногда не срабатывало автоматическое преобразования URL в ссылу
 ...
 *      + �?зменена обработка HTML-сущностей. Теперь все сущности имеющие эквивалент в Unicode (за исключением <>)
...
class Jevix
{
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Разрешение или запрет тегов
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Коротие теги типа <img>
     * @param array|string $tags тег(и)
     */
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Преформатированные теги, в которых всё заменяется на HTML сущности типа <pre>
     * @param array|string $tags тег(и)
     */
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Теги в которых отключено типографирование типа <code>
     * @param array|string $tags тег(и)
     */
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Не короткие теги которые не нужно удалять с пустым содержанием, например, <param name="code" value="die!"></param>
     * @param array|string $tags тег(и)
     */
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Тег необходимо вырезать вместе с контентом (script, iframe)
     * @param array|string $tags тег(и)
     */
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Добавление разрешённых параметров тега
    ...
    /**
     * КОНФ�?ГУРАЦ�?Я: Добавление необходимых параметров тега
     * @param string $tag тег
     * @param string|array $params разрешённые параметры
     */
    ...
    /* КОНФ�?ГУРАЦ�?Я: Установка тегов которые может содержать тег-контейнер
    * @param string $tag тег
    ...
    */
        ...
        foreach ($params as $param => $value) {
            ...
            if (is_string($paramAllowedValues)) {
                switch ($paramAllowedValues) {
                    ...
                    case '#text':
                        // cuca: &amp problems in href tags
    ...
    protected function quote($spacesBefore, &$quote, &$closed)
    {
        ...
        // �?ли, если открыто больше двух кавычек - точно закрываем
    ...
    protected function text(&$text)
    {
        ...
        while (($this->curCh != '<') && $this->curChClass) {
            ...
            if (!$spCount && $this->curCh == '&' && $this->htmlEntity($entity)) {
                ...
                // �?справляем ситуацию если кавычка закрыввается раньше чем открывается
